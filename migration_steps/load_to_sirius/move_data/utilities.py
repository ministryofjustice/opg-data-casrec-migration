...
def handle_special_cases(table_name, df):
    ...
    if table_name == "persons":
        ...
        log.debug("Reformat 'parent_id' to nullable int")
        df["parent_id"] = df["parent_id"].astype("Int64")
        log.debug("Reformat 'supervisioncaseowner_id' to nullable int")
        df["supervisioncaseowner_id"] = df["supervisioncaseowner_id"].astype("Int64")
        log.debug("Reformat 'executor_id' to nullable int")
        df["executor_id"] = df["executor_id"].astype("Int64")
        log.debug("Reformat 'deputynumber' to nullable int")
        df["deputynumber"] = df["deputynumber"].astype("Int64")
        log.debug("Reformat 'executivecasemanager_id' to nullable int")
        df["executivecasemanager_id"] = df["executivecasemanager_id"].astype("Int64")
        log.debug("Reformat 'firm_id' to nullable int")
