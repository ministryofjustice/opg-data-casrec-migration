...
def get_order_deputies_records(
    db_config,
    mapping_file_name,
    table_definition,
    sirius_details,
    ...
    """
    existing_deputies_df = pd.read_sql_query(existing_deputies_query, db_config["db_connection_string"])
    ...
    existing_deputies_merged_df = existing_deputies_merged_df.rename(columns={"id_y": "id"})
    ...
    order_query = f"""select "id", "c_cop_case" from {db_config["target_schema"]}.cases;"""
    ...
    deputyship_persons_joined_df["deputy_id"] = deputyship_persons_joined_df["deputy_id"].astype("Int64")
    ...
    return (deputyship_persons_order_df, False)
