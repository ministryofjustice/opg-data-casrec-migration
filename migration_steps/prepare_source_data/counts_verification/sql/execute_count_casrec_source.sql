...
ALTER TABLE countverification.counts DROP COLUMN IF EXISTS casrec_pre_migrate;
ALTER TABLE countverification.counts ADD COLUMN casrec_pre_migrate int;

-- persons_clients
UPDATE countverification.counts SET casrec_pre_migrate =
(
...
UPDATE countverification.counts SET casrec_pre_migrate =
(
    SELECT counts.casrec_pre_migrate FROM countverification.counts WHERE supervision_table = 'warnings'
)
