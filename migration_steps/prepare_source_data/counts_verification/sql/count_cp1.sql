...
SELECT id, caserecnumber INTO countverificationaudit.{working_column}_cases FROM countverification.cp1_cases;
UPDATE countverification.counts
SET {working_column} = (
...
SELECT bon.id, cp1_cases.caserecnumber INTO countverificationaudit.{working_column}_bonds
FROM bonds bon
...
SELECT sll.id, cp1_cases.caserecnumber INTO countverificationaudit.{working_column}_supervision_level_log
...
SELECT od.id, cp1_cases.caserecnumber INTO countverificationaudit.{working_column}_order_deputy
