...
CREATE SCHEMA IF NOT EXISTS pmf_order_deputy_in1201;
...
SELECT *
INTO pmf_order_deputy_in1201.order_deputy_updates
FROM od_updates;

--@audit_tag
SELECT od.*
INTO pmf_order_deputy_in1201.order_deputy_audit
FROM order_deputy od
INNER JOIN pmf_order_deputy_in1201.order_deputy_updates u
ON od.id = u.od_id;

--@update_tag
UPDATE order_deputy od
SET maincorrespondent = u.expected_maincorrespondent
FROM pmf_order_deputy_in1201.order_deputy_updates u
WHERE u.od_id = od.id;

--@validate_tag
SELECT od_id AS id, expected_maincorrespondent AS maincorrespondent
FROM pmf_order_deputy_in1201.order_deputy_updates
EXCEPT
