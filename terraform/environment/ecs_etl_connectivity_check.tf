resource "aws_ecs_task_definition" "etl_connectivity" {
  family                   = "etl-connectivity-${terraform.workspace}"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = 2048
  memory                   = 4096
  container_definitions    = "[${local.etl_connectivity}]"
  ...
  etl_connectivity = jsonencode({
    ...
    logConfiguration = {
      ...
      options = {
        ...
        awslogs-stream-prefix = "casrec-migration-etl-connectivity-${local.environment}"
      }
    },
    secrets = [
      {
