...
def generate_worksheet_data(module_name, mapping_dict):
    ...
    module_total_unmapped_rows = len(
        [
            ...
            if v["mapping_status"]["is_complete"] is not True
        ]
    )
    ...
    worksheet_data["total_unmapped"] = module_total_unmapped_rows
    worksheet_data["total_mapped"] = module_total_mapped_rows
    worksheet_data["percentage_complete"] = module_percentage_complete
...
def update_totals(worksheet_data, module_name, existing_total):
    ...
    sheets["total_complete"] = (
        sheets["total_complete"] + 1
        if worksheet_data["percentage_complete"] == 100
        else +0
    )
...
def add_module_to_summary(module_name, mapping_dict):
    ...
    summary["total"] = update_totals(
        worksheet_data, module_name, existing_total=summary["total"]
    )
