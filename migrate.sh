...
docker-compose run --rm --name casrec_load_1 load_casrec prepare/prepare.sh >> docker_load.log &
P1=$!
sleep 1
docker-compose run --rm --name casrec_load_2 load_casrec prepare/prepare.sh >> docker_load.log &
P2=$!
sleep 1
docker-compose run --rm --name casrec_load_3 load_casrec prepare/prepare.sh >> docker_load.log &
P3=$!
sleep 1
docker-compose run --rm --name casrec_load_4 load_casrec prepare/prepare.sh >> docker_load.log &
P4=$!
wait $P1 $P2 $P3 $P4
cat docker_load.log
rm docker_load.log
