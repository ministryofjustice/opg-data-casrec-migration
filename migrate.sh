...
docker-compose ${COMPOSE_ARGS} run --rm --name casrec_load_1 load_casrec python3 app.py --delay=0 --skip_load="${SKIP_LOAD}" >> docker_load.log &
P1=$!
docker-compose ${COMPOSE_ARGS} run --rm --name casrec_load_2 load_casrec python3 app.py --delay=2 --skip_load="${SKIP_LOAD}" >> docker_load.log &
P2=$!
docker-compose ${COMPOSE_ARGS} run --rm --name casrec_load_3 load_casrec python3 app.py --delay=3 --skip_load="${SKIP_LOAD}" >> docker_load.log &
P3=$!
docker-compose ${COMPOSE_ARGS} run --rm --name casrec_load_4 load_casrec python3 app.py --delay=4 --skip_load="${SKIP_LOAD}" >> docker_load.log &
P4=$!
wait $P1 $P2 $P3 $P4
cat docker_load.log
rm docker_load.log
echo "=== Step 1 - Transform ==="
docker-compose ${COMPOSE_ARGS} run --rm transform_casrec transform_casrec/transform.sh --team="${LAY_TEAM}" --clear=True
echo "=== Step 2 - Integrate with Sirius ==="
docker-compose ${COMPOSE_ARGS} run --rm integration integration/integration.sh
